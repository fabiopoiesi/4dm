...
namespace Reconstruction4D
{
    ...
    /// </summary>
    public class MultiplatformMeshSelector : NetworkedBehaviour
    {
        public NetworkedVar<string> creatorClientId = new NetworkedVar<string>(string.Empty);
        ...
        public void Start()
        {
            ...
            // Inform server that star / anchor finish showing (execute this only if you are on a different macchine)
            if (creatorClientId.Value != SystemInfo.deviceUniqueIdentifier)
            {
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>().ReceiveStar(this.networkId);
            }
        }
    }
}
