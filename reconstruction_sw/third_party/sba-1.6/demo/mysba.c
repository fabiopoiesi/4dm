...
void sba_driver(char *camsfname, char *ptsfname, char *calibfname, int cnp, int pnp, int mnp,
                ...
                int filecnp, char *refcamsfname, char *refptsfname, double* res, int howto, int verbose,
                float* error_init, float* error_end, float* elapsed_time)
{
  ...
  int expert, analyticjac, fixedcal, havedist, n, prnt;
  int nframes, numpts3D, numprojs, nvars;

  const int nconstframes = 0; // CAMERAS THAT WILL NOT BE BUNDLED STARTING FROM THE FIRST
  ...
  if(calibfname){ // read intrinsics only if fixed for all cameras
     ...
     */
    globs.nccalib = 5; /* number of intrinsics to keep fixed, must be between 0 and 5 */
  ...
  /* call sparse LM routine -> defined in sba.h */ 
  opts[0]=SBA_INIT_MU; 
  ...
  memcpy(res, motstruct, (nframes*cnp + numpts3D*pnp)*sizeof(double)); // save data to return

  *elapsed_time = ((float) (end_time - start_time)) / CLOCKS_PER_MSEC;
  *error_init = info[0]/numprojs;
  *error_end = info[1]/numprojs;
  ...
  fprintf(stdout, "SBA returned %d in %g iter, reason %g, error %g [initial %g], %d/%d func/fjac evals, %d lin. systems\n", n,
                    info[5], info[6], *error_end, *error_init, (int)info[7], (int)info[8], (int)info[9]);
  fprintf(stdout, "Elapsed time: %.2lf seconds, %.2lf msecs\n", ((double) (end_time - start_time)) / CLOCKS_PER_SEC, *elapsed_time);
  fflush(stdout);
...
void testlib()
{
  printf("libmysba.so is working\n");
}


void sba(char* cam, char* pts, double* res, int howto, int verbose, float* error_init, float* error_end, float* elapsed_time)
{
  ...
  sba_driver(cam, pts, NULL, cnp, pnp, mnp, quat2vec, vec2quat, cnp+1, "-", "-", res, howto, verbose, error_init, error_end, elapsed_time);
}
