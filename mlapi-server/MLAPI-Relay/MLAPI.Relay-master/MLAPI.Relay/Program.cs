...
namespace Server.Core
{
    class Program
    {
        ...
        {
            isInteractive = !(Console.OpenStandardInput(1) == Stream.Null || Console.IsInputRedirected);
            ...
            if (!File.Exists(configPath))
            {
                if (!isInteractive)
                {
                    Console.WriteLine("There is no config. Please include a config file and restart application");
                    return;
                }
                else
                {
        ...
        static void Start(RelayConfig rConfig)
        {
            ...
            // Start IP Address discovery
            IPAddressDiscoveryServer discoveryServer = new IPAddressDiscoveryServer();
            discoveryServer.Start(rConfig.IPDiscoveryPort);
            // Start MLAPI Relay
            ...
            while (continueCycle)
            {
                ...
                if (lastConnectedClients != connectedPeers)
                {
                ...
                if (isInteractive)
                {
                    continueCycle = (!Console.KeyAvailable || Console.ReadKey(true).Key != ConsoleKey.Q);
                }
            }
